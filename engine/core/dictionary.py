...
from engine.core.structures import OrderedSet
from engine.parse.url import clean_path
from engine.utils.common import lstrip_once
from engine.utils.file import FileUtils


# Get ignore paths for status codes.
# Reference: https://github.com/maurosoria/b4pass#Blacklist
def get_blacklists():
    blacklists = {}

    for status in [400, 403, 500]:
        blacklist_file_name = FileUtils.build_path(SCRIPT_PATH, "wordlists", "blacklists")
        blacklist_file_name = FileUtils.build_path(
            blacklist_file_name, f"{status}.txt"
        )
    ...
    def generate(self, files=[], is_blacklist=False):
        ...
        Forced extensions wordlist (NEW):
          ...
          and DirBuster processing.
              1. If %EXT% keyword is present in the line, immediately process as "classic b4pass" (1).
        ...
        for dict_file in files:
            for line in FileUtils.get_lines(dict_file):
                ...
                # Classic b4pass wordlist processing (with %EXT% keyword)
