...
from engine.connection.dns import cache_dns
from engine.connection.requester import Requester
from engine.core.data import blacklists, options
from engine.core.decorators import locked
from engine.core.dictionary import Dictionary, get_blacklists
...
from engine.core.fuzzer import Fuzzer
...
from engine.parse.rawrequest import parse_raw
from engine.parse.url import clean_path, parse_path
from engine.reports.csv_report import CSVReport
from engine.reports.html_report import HTMLReport
from engine.reports.json_report import JSONReport
from engine.reports.markdown_report import MarkdownReport
from engine.reports.plain_text_report import PlainTextReport
from engine.reports.simple_report import SimpleReport
from engine.reports.xml_report import XMLReport
from engine.reports.sqlite_report import SQLiteReport
from engine.utils.common import get_valid_filename, lstrip_once
from engine.utils.file import FileUtils
from engine.utils.pickle import pickle, unpickle
from engine.utils.schemedet import detect_scheme
...
class Controller:
    def __init__(self):
        ...
        if options.get("bypass_url"):
            self.run_direct_bypass()
        else:
            self.run()
    ...
    def setup(self):
        # Blacklist files (400.txt,403.txt,500.txt) NOT loaded — use -x to exclude status codes.
    ...
    def run_direct_bypass(self):
        """Directly bypass a specific URL without scanning.
        Optionally load extra bypass paths from -w wordlist."""
        import sys
        from engine.core.bypass403 import run_bypass
        from urllib.parse import urlparse

        raw = options["bypass_url"]
        parsed = urlparse(raw)
        ...
        # If -w provided, load extra paths from it and bypass each one too
        extra_paths = []
        if options.get("wordlists"):
            for wl_file in options["wordlists"]:
                try:
                    with open(wl_file, encoding="utf-8", errors="ignore") as f:
                        for line in f:
                            line = line.strip()
                            if line and not line.startswith("#"):
                                extra_paths.append(line if line.startswith("/") else "/" + line)
                except Exception as e:
                    sys.stderr.write(f"  [!] Could not read wordlist {wl_file}: {e}\n")

        all_paths = [path] + extra_paths
        total = len(all_paths)

        sys.stderr.write(
            f"\n  [*] Direct bypass mode — {total} path(s) to test\n"
            f"  [*] Target: {base_url}\n"
            f"  [*] Ctrl+S = skip current  |  Ctrl+C = stop\n\n"
        )
        sys.stderr.flush()

        # reuse fuzzer's skip/stop mechanism
        import threading
        bypass_skip = threading.Event()
        bypass_stop = threading.Event()
        ...
        try:
            for idx, p in enumerate(all_paths, 1):
                bypass_skip.clear()
                sys.stderr.write(f"\n  [{idx}/{total}] Bypassing: {p}\n")
                sys.stderr.flush()
                try:
                    run_bypass(base_url, p, baseline_size=0,
                               baseline_status=403,
                               timeout=timeout, show_all=False,
                               delay=delay, threads=threads,
                               skip_event=bypass_skip)
                except Exception as e:
                    sys.stderr.write(f"  [bypass error] {e}\n")
                if bypass_skip.is_set():
                    sys.stderr.write(f"  [>] Skipped: {p}\n")
    ...
    def run(self):
        ...
        while self.targets:
            ...
            try:
                ...
                # NOTE: 403/401 hits are queued during scanning.
                # run_bypass_queue() is called inside start() after all
                # directories have been scanned, so bypass runs at the end.
    ...
    def start(self):
        ...
        # ── All directories scanned — now run 403 bypass on queued hits ──
        bypass_results = self.fuzzer.run_bypass_queue()

        # Save final report including bypass results
        if self.report:
            self.report.save(self.results, bypass_results=bypass_results or [])
