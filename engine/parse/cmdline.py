...
def parse_arguments():
    ...
    parser = OptionParser(usage, version=f"b4pass v{VERSION}")
    ...
    mandatory.add_option(
        ...
        dest="bypass_url",
        metavar="URL",
        help="Directly bypass a specific URL/path (skips scanning). Use with -w for custom bypass wordlist.",
    )
    mandatory.add_option(
        "-l",
        "--url-file",
    ...
    dictionary.add_option(
        ...
        help="Add extensions to the end of every wordlist entry. By default b4pass only replaces the %EXT% keyword with extensions",
    )
    dictionary.add_option(
        "-O",
    ...
    request.add_option(
        "--data", action="store", dest="data", help="HTTP request data"
    )
    request.add_option(
        "--data-file",
        action="store",
    ...
    connection.add_option(
        ...
        dest="delay",
        metavar="DELAY",
        help="Delay between requests (seconds), respected by both the "
             "main scan and the 403 bypass battery",
    )
    connection.add_option(
        "--proxy",
        action="append",
    ...
    advanced.add_option(
        "--skip-soft-wall-bypass",
        action="store_true",
        dest="skip_soft_wall_bypass",
        help="Don't run the bypass battery against 200s flagged as a "
             "client-side/SPA soft auth-wall (on by default; can multiply "
             "request volume on scans with many such hits)"
    )
